...
/* ⚠️ Cambia esto por cualquier texto largo y difícil de adivinar (por ejemplo,
   ...
   pongas en SHARED_SECRET dentro de google-apps-script.gs — así el script/* ⚠️ Reemplaza esto por tu usuario real de Instagram (sin @) */
const IG_USERNAME = 'nikkoppon';
...
function renderCart(){
  ...
  if(appliedDiscount){
    ...
    document.getElementById('discountAppliedDetail').textContent = appliedDiscount.tipo === 'cantidad_fija'
      ? appliedDiscount.cantidad + ' ' + appliedDiscount.producto + ' por ' + formatCLP(appliedDiscount.precioFijo)
...
function getDiscountAmount(subtotal){
  ...
  if(appliedDiscount.tipo === 'cantidad_fija'){
    ...
    units.sort((a,b)=>b-a); // las más caras primero — así el cliente se beneficia más

    const bundles = Math.floor(units.length / appliedDiscount.cantidad);
    let discount = 0;
    for(let b=0; b<bundles; b++){
      const group = units.slice(b*appliedDiscount.cantidad, (b+1)*appliedDiscount.cantidad);
      const groupNormalTotal = group.reduce((s,p)=>s+p, 0);
      discount += groupNormalTotal - appliedDiscount.precioFijo;
    }
...
const HERO_SLIDE_SECONDS = 5; // cuánto dura cada foto antes de cambiar sola
let heroIndex = 0;
let heroTimer = null;
...
function goToHeroSlide(index){
  showHeroSlide(index);
  startHeroAutoplay(); // reinicia la cuenta al elegir una foto puntual
}

// Botones de flecha (‹ ›) — el usuario puede volver atrás si quiere,
// y esto también reinicia el tiempo antes del próximo cambio automático.
function heroSlideStep(delta){
  showHeroSlide(heroIndex + delta);
  startHeroAutoplay();
}
